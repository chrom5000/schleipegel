...
- **hero3d-Datenlayer**: DOM-Marker (`maplibregl.Marker`) — überleben `setStyle` und liegen über 3D und Seekarte. hero3d liest die app.js-Globals (`state`, `classify`, `bftClass`, Formatter …) direkt; app.js ruft `window.HERO3D?.renderData?.()` am Ende von `renderTiles()`/`renderRevierWind()` und `renderLight?.()` in `renderSunLayer()`. Falle: `rotationAlignment:'map'` zieht `pitchAlignment:'map'` nach — für aufrechte Billboards explizit `pitchAlignment:'viewport'` setzen; Windzahlen werden um `bearing − dir` gegenrotiert (Update im `rotate`-Event).
